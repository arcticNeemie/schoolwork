...
Consider the two distance measures below:
\begin{algorithm}[H]
...
\end{algorithmic}
\end{algorithm}
These are all applied in lines 8-9 of Algorithm \ref{alg:kNN}, which are repeated $n$ times. Here, rather than focus on the loop in Algorithm \ref{alg:kNN}, we will focus on applying Foster's Design Methodology to each sorting algorithm individually.
