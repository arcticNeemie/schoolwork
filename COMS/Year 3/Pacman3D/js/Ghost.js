function makeGhost(colour,specularHighlight){
    ...
    var cloudHead = makeHeadCloud(cloudMaterial);
    var cloudBody = makeBodyCloud(cloudMaterial);
    ghost.add(head);
    ghost.add(body);
    ghost.add(leftEye);
    ghost.add(rightEye);
...
function animateGhost(ghost){
    ghost.children[4].rotateY(0.1);
    ghost.children[5].rotateY(0.1);
}
...
function makeEye(){
    ...
    var eyeMaterial = new THREE.MeshPhongMaterial(
        ...
        specular: 0x222222,
        shininess: 32});
    makeTexture("resources/eyeTex.png",eyeMaterial);
